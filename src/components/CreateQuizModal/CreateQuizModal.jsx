...
export default function CreateQuizModal({
  ...
  async function createQuiz() {
    try {
      const questionList = questionsRefs.current.map((ref) =>
        ref?.getQuestionData()
      );
  ...
  return (
    <div className="absolute inset-0 bg-black/50 flex items-center justify-center z-9999 ">
      <div className="relative w-[50%] h-[90%] bg-white rounded-[8px] shadow-black shadow-md pb-[96px] overflow-y-auto ">
        <div className="sticky top-0 right-0 flex justify-end p-4 bg-white z-10">
          <ExitIcon
            onClick={() => {
              setShowCreateQuiz(false);
            }}
            className="text-[24px] hover:scale-120 transtion-all duration-500"
          ></ExitIcon>
        </div>
        <div className="flex flex-row gap-4 w-full items-center justify-center pt-2">
          ...
          <input
            type="text"
            className="flex-grow mr-8 text-2xl py-2 px-4 rounded-[8px]"
            value={quizName}
            onChange={(e) =>
              setQuizName((prev) => {
                return prev;
        ...
        <div className="flex flex-row gap-4 w-full items-center justify-center pt-2">
          ...
          <div className="flex-grow">
            <input
              type="text"
              className="w-[80%] mr-8 text-2xl py-2 px-4 rounded-[8px]"
            ...
            <ExpandButton
              ...
              className={`text-4xl hover:scale-120 transition-alll duration-500 ${
        ...
        <div
          className={`mt-4 mx-auto rounded-[8px] w-[90%] ${
            expandChapter ? "h-[348px]" : "h-0"
          } shadow-sm bg-gray-50 shadow-black transition-all duration-500 overflow-y-auto`}
        >
          {chapters?.map((chapter, i) => {
            ...
            return (
              <div
                onClick={() => {
                  handleOnClickChapter(chapter);
                }}
                key={`chapter_${i}`}
                className="transition-all duration-200 mt-4 hover:bg-[#75b971] text-gray-700 hover:text-white flex items-center pt-4 mx-auto w-[95%]  rounded-[8px] bg-white shadow-sm shadow-gray-700 px-4 "
              >
        ...
        {Array.from({ length: questionCount }).map((_, i) => {
          return (
            <NewQuestion
              key={`question_${i}`}
              ref={(element) => (questionsRefs.current[i] = element)}
              index={i}
            ></NewQuestion>
          );
        })}
      ...
      {showAlert && (
        <DefaultAlert
          ...
          information="Đã tạo đề thành công, hãy nhấn đóng để về trang quản lý"
          closeButton={() => {
...
const NewQuestion = forwardRef((props, ref) => {
  ...
  return (
    <div className="relative mt-4 mx-auto w-[90%] h-[300px] px-4  border-1 rounded-[8px] flex-col overflow-y-auto pb-4">
        ...
        <>
          <div className="flex flex-row mt-4 items-center ">
            ...
            <input
              ...
              type="text"
              value={imageURL}
              className="flex-grow px-4 py-2 rounded-[8px] "
            ></input>
          </div>
        </>
      )}
      ...
      <div className=" absolute right-4 flex flex-row gap-4">
        <button
          ...
          className="mt-4 text-white bg-[#3D763A] px-4 py-2 rounded-[8px] "
        >
          Thêm lựa chọn
        </button>
      </div>
      ...
      <div className="flex flex-row mt-4 items-center ">
        ...
        <select
          ...
          name="optionsList"
          className="flex-grow max-w-[85%] bg-white px-4 py-2 rounded-[8px]"
        >
          {options.map((option, index) => {
            return (
              <option key={`option_select_${index}`} value={option}>
                {option}
              </option>
            );
          })}
